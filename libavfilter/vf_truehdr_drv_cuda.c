 ...
 * from vf_truehdr_cuda, which runs the NGX SDK snippet (nvngx_truehdr.dll).  The
 ...
 * launches.  truehdr_drv_cuda_gen.h encodes how the whole graph (grids, scratch
 ...
 * calculate_pov scene stat and is shaped by the tunable floats.  This network is
...
/* Tone-curve presets (the `preset` option).  A preset names a whole tunable set;
 * "sdk" makes this (much brighter) driver network approximate the SDK truehdr_cuda
 * filter's tone curve. */
enum {
    THDRV_PRESET_NONE = 0,
    THDRV_PRESET_SDK  = 1,
...
typedef struct TrueHdrDrvCudaContext {
    ...
    /* options */
    double maxluminance;
    int    preset;                ///< tone-curve preset (THDRV_PRESET_*), applied over the tunables
    int    tonemap;               ///< 0 = byte-exact bypass; 1 = adaptive inverse-tone-map
    double contrast, saturation, middlegray, exposure, gamma;
    char  *out_format;            ///< "rgbaf16le" (scRGB) or "x2bgr10le" (HDR10 PQ)
    int    gamut;                 ///< x2bgr10le: 1 = Rec.2020 primaries (HDR10), 0 = Rec.709
    int    experimental_arch;     ///< allow the unverified non-Blackwell (sm_75) path
    char  *data_dir;
...
static const AVOption truehdr_drv_cuda_options[] = {
    ...
    /* preset: a named tunable set.  The driver network is a different, much brighter
     * net than the SDK truehdr_cuda filter; preset=sdk applies values (found by
     * sweeping this filter against the SDK filter on real content) that pull its
     * adaptive curve onto the SDK's -- tonemap=1, exposure=800, middlegray=15 (14 for
     * PQ output, which weights midtones more).  It only fills in the tunables left
     * at "auto" (-1), so e.g. preset=sdk:exposure=600 keeps the explicit exposure. */
    { "preset", "tone-curve preset over the tunables: none, or sdk (emulate the SDK truehdr_cuda curve)",
      OFFSET(preset), AV_OPT_TYPE_INT, {.i64=THDRV_PRESET_NONE}, 0, 1, FLAGS, .unit = "preset" },
        { "none", "use the tunables directly (no preset)",       0, AV_OPT_TYPE_CONST, {.i64=THDRV_PRESET_NONE}, 0, 0, FLAGS, .unit = "preset" },
        { "sdk",  "emulate the SDK truehdr_cuda tone curve",      0, AV_OPT_TYPE_CONST, {.i64=THDRV_PRESET_SDK},  0, 0, FLAGS, .unit = "preset" },
    /* tonemap 0 = the captured ToneMapMode-0 bypass (near-linear, reads only
     * MaxLuminance) -- byte-exact vs loader_ppe.  tonemap 1 = the driver's adaptive
     * inverse-tone-map, which consumes the live per-frame calculate_pov scene stat
     * (fraction of bright pixels) and is shaped by the tone floats below.  The exact
     * production values the driver's DXVA/PPE config passes are not observable from
     * the loader (we are the caller), so these are exposed with neutral defaults;
     * the curve rolls highlights off toward, not to, the peak (an inverse-tone-map,
     * not a linear scale). Named by live value-correlation (rtx-video-re
     * docs/drtm610/). */
    /* The three tunables a preset names default to -1 = auto: the preset's value
     * if one is selected, else the neutral one quoted in the help text.  A
     * sentinel rather than the neutral value itself, so that passing that value
     * explicitly is distinguishable from not passing it at all. */
    { "tonemap", "tone-map algorithm: -1=auto (1), 0=bypass (byte-exact), 1=adaptive inverse-tone-map",
      OFFSET(tonemap), AV_OPT_TYPE_INT, {.i64=-1}, -1, 1, FLAGS },
    { "contrast",   "adaptive tone-curve contrast (tonemap=1)",              OFFSET(contrast),   AV_OPT_TYPE_DOUBLE, {.dbl=1.0},  0.1, 4.0,   FLAGS },
    { "saturation", "adaptive chroma saturation (tonemap=1; 1=neutral)",     OFFSET(saturation), AV_OPT_TYPE_DOUBLE, {.dbl=1.0},  0.0, 4.0,   FLAGS },
    { "middlegray", "adaptive tone-curve middle-gray reference (tonemap=1; -1=auto, neutral 50)", OFFSET(middlegray), AV_OPT_TYPE_DOUBLE, {.dbl=-1.0}, -1.0, 100.0, FLAGS },
    { "exposure",   "adaptive highlight-expansion gain (tonemap=1; -1=auto, neutral 200)",        OFFSET(exposure),   AV_OPT_TYPE_DOUBLE, {.dbl=-1.0}, -1.0, 1000.0,FLAGS },
    /* Per-channel gamma (drtm arg0x10, out = scale*ch^gamma).  Inert until enabled
     * by arg0x40 AND ToneMapMode 1; the filter enables it only when gamma!=1 (so the
     * default leaves the mode-1 arg buffer untouched).  1.0 = identity. */
    { "gamma",      "per-channel output gamma (tonemap=1; 1.0=off)",          OFFSET(gamma),      AV_OPT_TYPE_DOUBLE, {.dbl=1.0},  0.25,4.0,   FLAGS },
    ...
    { "format", "output: rgbaf16le=scRGB linear 80nit (default), x2bgr10le=HDR10 PQ",
      OFFSET(out_format), AV_OPT_TYPE_STRING, {.str="rgbaf16le"}, 0, 0, FLAGS },
    /* x2bgr10le only: apply the drtm Rec.709->Rec.2020 gamut matrix (arg0x2b) so
     * the PQ output is standards-correct HDR10 / BT.2100.  0 keeps Rec.709 primaries
     * (PQ-over-709).  Ignored for the linear rgbaf16le/scRGB output (Rec.709). */
    { "gamut", "x2bgr10le: 1=Rec.2020 primaries (HDR10, default), 0=keep Rec.709",
      OFFSET(gamut), AV_OPT_TYPE_BOOL, {.i64=1}, 0, 1, FLAGS },
    /* The truehdr_* glue cubins ship as multi-arch fatbins (sm_75/sm_80/sm_120/
     ...
     * `rtxv fatbins --cross-major` pairs them across the major boundary by three
     * agreeing static signals (.text rank, ISETP output-size bounds, SASS immediate
     * fingerprint), yielding sm_75/sm_86/sm_87/sm_89/sm_120 -- so Turing/Ampere/Ada
     * all get k_conv images.  sm_89 (Ada) is confirmed byte-identical to the DLL on
     * an RTX 4060 Ti and runs ungated; the other statically-matched arches remain
     * UNVERIFIED on real hardware, so this opt-in lifts the guard for those GPUs. */
    { "experimental_arch", "attempt the statically-matched but unverified non-Blackwell path (sm_89/Ada does not need this)",
      OFFSET(experimental_arch), AV_OPT_TYPE_BOOL, {.i64=0}, 0, 1, FLAGS },
    { "data", "directory with extracted driver TrueHDR cubins + weights.bin",
      OFFSET(data_dir), AV_OPT_TYPE_STRING, {.str=TRUEHDRDRV_DEFAULT_DATA_DIR}, 0, 0, FLAGS },
    { NULL }
};
...
static const FFRtxArchGate thdrv_gate = {
    .gate_msg =
        "truehdr_drv_cuda is validated on Blackwell (cc 12.x) and Ada (cc 8.9); "
        "this GPU is cc %d.%d.  Its k_conv is statically matched but unverified "
        "-- set experimental_arch=1 to attempt it anyway.\n",
    .warn_msg =
        "truehdr_drv_cuda: EXPERIMENTAL non-Blackwell (cc %d.%d) path -- k_conv is "
...
static int setup_graph(AVFilterContext *ctx)
{
    ...
    int Ha = (H + 15) & ~15;   /* conv padding rounds height to a multiple of 16 */
    int sdk, tonemap;
    ...
    if ((ret = ff_rtx_arch_gate(ctx, &s->r, &thdrv_gate, s->experimental_arch)) < 0)
        return ret;
    if ((ret = ff_rtx_load_modules(ctx, &s->r, s->data_dir,
                                   (const FFRtxModule *)thdrv_modules, THDRV_NMODULE, THDRV_MAX_MID,
    ...
    nup = thdrv_fill_uploads(W, H, NW, NH, (const thdrv_devptr *)s->r.alloc, up);
    ...
    if (thdrv_fill_graph(W, H, NW, NH, (const thdrv_devptr *)s->r.alloc,
                         (thdrv_devptr)s->in_img->tex, (thdrv_devptr)s->out_img->surf,
    ...
    middlegray = s->middlegray >= 0 ? s->middlegray
               : sdk ? (s->outpf->f == AV_PIX_FMT_X2BGR10LE ? 14.0 : 15.0) : 50.0;
    if (sdk)
    ...
    av_log(ctx, AV_LOG_INFO,
           "driver TrueHDR graph ready: %dx%d (NN %dx%d)  %s -> %s  (%d launches, %d buffers)  "
           "tonemap=%s\n",
...
static void retag_hdr(AVFilterContext *ctx, AVFrame *out)
{
    ...
    out->color_primaries = (hdr10 && s->gamut) ? AVCOL_PRI_BT2020 : AVCOL_PRI_BT709;
...
const FFFilter ff_vf_truehdr_drv_cuda = {
    ...
    .p.description = NULL_IF_CONFIG_SMALL("NVIDIA RTX driver TrueHDR SDR-to-HDR (CUDA)"),
