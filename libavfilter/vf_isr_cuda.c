 ...
 * network (the snippet NvDLISR/nvngx_dlisr.dll shipped with the driver / NVIDIA
 * App).  Distinct from vf_vsr_cuda and vf_dlpp_drv_cuda, which run the *video*
 * super-resolution networks: ISR is a fidelity-oriented still-image upscaler at
 ...
 * (rtx-video-re docs/FINDINGS-dlisr.md).  Unlike the VSR graphs, ISR's network is
 * RESOLUTION-INDEPENDENT: it runs on fixed 256x256 tiles, so all of its per-tile
 * launches carry identical grids, blocks and argument bytes at every input size
 ...
 * quantities, verified against captures at many resolutions and all three scales.
 *
 * Pipeline (see isr_cuda_gen.h):
 *
 *      convertTypetoHalf         whole image, RGBA8 -> fp16 NHWC
 *      splitTilesType            -> batch of 256x256 tiles, stride 240 (overlap 16)
 *      for each tile:
 ...
 *
 * The conversion and tiling kernels are CUDA-runtime (__global__) functions and
 ...
 * resampling path, so unlike the VSR filters there is no arbitrary output size:
 * the output is always scale x the input.  Input/output are plain linear device
 * buffers (not textures/surfaces), so frames are staged through packed scratch.
...
static int setup_graph(AVFilterContext *ctx)
{
    ...
    if ((ret = ff_rtx_load_modules(ctx, &s->r, s->data_dir,
                                   (const FFRtxModule *)c->modules, c->nmod, ISR_MAX_MID,
                                   (const FFRtxFunc *)c->funcs, c->nfunc, ISR_MAX_FID,
                                   ISR_LOAD_HINT)) < 0)
        return ret;
    ...
    ret = ff_rtx_upload_weights(ctx, &s->r, s->data_dir, c->weights, up, c->nupload);
    av_freep(&up);
    if (ret < 0)
        return ret;
...
static int config_output(AVFilterLink *outlink)
{
     ...
     * R<->B swap, so only the shared table's R-first 8-bit rows fit.  Output is
     * the input format. */
...
const FFFilter ff_vf_isr_cuda = {
    .p.name        = "isr_cuda",
    .p.description = NULL_IF_CONFIG_SMALL("NVIDIA NGX Image Super Resolution (2x/4x/8x) in CUDA"),
